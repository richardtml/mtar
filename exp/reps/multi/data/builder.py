...
def build_dataloader(datasets_dir, ds, split, subset, batch_size,
  ...
  ds = Dataset(ds_dir, split=split, subset=subset,
      min_seq=min_seq, max_seq=max_seq, cache=cache,
      sampling=sampling, verbose=verbose, print_dropped=print_dropped)
  dl = DataLoader(ds, batch_size=batch_size,
      shuffle=shuffle, num_workers=0 if cache else num_workers)
  return dl
